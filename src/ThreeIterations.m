...
clc;
format long
% This is the main file for running three iterations.
...
X_total_real = [round(real(X_total),5); round(imag(X_total),5)];
...
%     T2_counter = 1;
    ...
    for j = 1:2*symbolSize:length(data)
        ...
        % Decoder
        R(3:4) = conj(R(3:4));
        x_decoded_hat= decoder(H_est_ls, R);
        ...
        %Back into the precoder to produce T.
        t1_bar = [x_bar_prime(1); x_bar_prime(2)]; % Assume accurate decoder
        t2_bar = [conj(-x_bar_prime(2)); conj(x_bar_prime(1))]; % Assume accurate decoder
        T = [t1_bar; t2_bar];
        ...
        % Decoder from second network output.
        x_decoded_bar_prime_prime = decoder(H_bar, R);
        
        % First network Again, final prediction
        pred = predict(net,[real(x_decoded_bar_prime_prime); imag(x_decoded_bar_prime_prime)]);
        x_bar_prime_prime = [pred(1) + pred(3)*i; pred(2) + pred(4)*i];
        ...
        prediction3(pred1and2_counter : pred1and2_counter+1) = x_bar_prime_prime;
    ...
    outputVector =  convertToBits( prediction3, numSymbols);
    numberOfBitErrors = sum(abs(outputVector -  data));
    BER = numberOfBitErrors/size(data,1)
    Percent_Accuracy = (1 -BER )*100;
    BER_prediction3 = [BER_prediction3, BER];
...
hold on;
semilogy(SNR,BER_prediction3)
legend("Black Box", "Iterative", "Three Iterations")
...
tableVales = [SNR' ,BER_prediction1' , BER_prediction2' ,BER_prediction3' ]
